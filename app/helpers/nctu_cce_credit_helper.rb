module NctuCceCreditHelper
  ...
  [:nctu_cce_feedback_1_1, :nctu_cce_feedback_1_2, :nctu_cce_feedback_1_3, :nctu_cce_feedback_1_4, :nctu_cce_feedback_1_5, 
   ...
   :nctu_cce_feedback_2_11].each do |f|
    class_eval %Q{  
    def #{f}SubFeedbackStatistics(item, sub_item_id)
      result = Array.new(12, 0.0)
      temp_data = Array.new(6, 0.0) 
      user_count = 0
      item.progresses.where(stage: 5, feedback_done: true).each do |p|     
        p.registered_sub_items.each do |rsi| 
          if rsi.sub_item.id == sub_item_id           
            user_count = user_count + 1    
            result[0] =result[0] + rsi.#{f}
            case rsi.#{f}
            when 5
              result[1] =result[1] + 1
            when 4
            ...
            case rsi.nctu_cce_feedback_4_1
            when 0
              temp_data[0] = temp_data[0] + 1
              result[6] = result[6] + rsi.#{f}
            when 1
              temp_data[1] = temp_data[1] + 1    
              result[7] = result[7] + rsi.#{f}     
            when 2            
              temp_data[2] = temp_data[2] + 1    
              result[8] = result[8] + rsi.#{f}                  
            end
            case rsi.nctu_cce_feedback_4_4
            when 0
              temp_data[3] = temp_data[3] + 1
              result[9] = result[9] + rsi.#{f}
            when 1
              temp_data[4] = temp_data[4] + 1    
              result[10] = result[10] + rsi.#{f}          
            when 2            
              temp_data[5] = temp_data[5] + 1    
              result[11] = result[11] + rsi.#{f}                   
            end            
          end   
        end   
      end         
